...
if exist "wrapper\config-offline.json" (
	echo ^(5^) Online lVM is[92m ON [0m
) else ( 
	echo ^(5^) Online lVM is[91m OFF [0m
)
:: Skip updating
if !AUTOUPDATE!==y (
...
if "!choice!"=="?5" (
	echo By default, the latest online lvm will be disabled.
	echo While useful, it may take some important features away from the lvm such as asset related stuff.
	echo only use this feature if you really want to test the new lvm online.
	goto reaskoptionscreen
)
:: Auto Update
if "!choice!"=="6" (
...
if "!choice!"=="?6" (
	echo By default, when you open start_redrawn.bat it 
...
if "!choice!"=="?8" (
    ...
    echo:
	echo It's used to show when you're using Redrawn in your "Playing A Game" status on Discord,
...
if exist "server\characters\characters.zip" (
	...
	if "!choice!"=="?9" (
		echo When first getting Redrawn, all non-stock characters are put into a single zip file.
